...
import { User, ShieldCheck, Mail, Trash2, UserCog, Search, CheckCircle2, XCircle, CalendarDays } from "lucide-react";
...
export default function AdminUsers() {
    ...
    const handleUpdateUser = async (id: string, updates: { role?: string, status?: boolean }) => {
        try {
            await updateUserRole(id, updates);
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-16">
                    <div>
                        <div className="flex items-center gap-3 text-teal-400 mb-2 font-black uppercase tracking-[0.4em] text-[10px]">
                            <ShieldCheck size={14} /> Security & Governance
                        </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-white/5 text-gray-500 text-[10px] uppercase font-black tracking-[0.2em]">
                                    ...
                                    <th className="px-10 py-8">Account Access</th>
                            ...
                            <tbody className="divide-y divide-white/5">
                                {loading ? (
                                    <tr><td colSpan={5} className="px-10 py-32 text-center text-gray-500 font-bold tracking-widest uppercase animate-pulse">Synchronizing Users...</td></tr>
                                    ...
                                    <tr key={user.id} className="hover:bg-white/[0.02] transition-colors group">
                                        ...
                                        <td className="px-10 py-6">
                                            <select
                                                ...
                                                className={`bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-full outline-none transition-all cursor-pointer ${user.role === 'ADMIN' ? 'text-purple-400' : user.role === 'SELLER' ? 'text-teal-400' : 'text-blue-400'
                                                    }`}
                                            >
                                                <option value="CUSTOMER" className="bg-[#050b18]">Customer</option>
                                                <option value="SELLER" className="bg-[#050b18]">Seller</option>
                                                <option value="ADMIN" className="bg-[#050b18]">Admin</option>
                                            </select>
                                        </td>

                                        <td className="px-10 py-6">
                                            <button
                                                onClick={() => handleUpdateUser(user.id, { status: !user.status })}
                                                className={`flex items-center gap-2 px-4 py-2 rounded-full border text-[10px] font-black uppercase tracking-widest transition-all ${user.status
                                                    ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/20'
                                                    : 'bg-red-500/10 border-red-500/30 text-red-400 hover:bg-red-500/20'
                                                    }`}
                                            >
                                                {user.status ? <CheckCircle2 size={13} /> : <XCircle size={13} />}
                                                {user.status ? 'Active' : 'Banned'}
                                            </button>
                                        </td>

                                        <td className="px-10 py-6">
                                            <p className="text-xs font-mono text-gray-500 flex items-center gap-2 italic">
                                        ...
                                        <td className="px-10 py-6 text-right">
                                            <div className="flex justify-end gap-3">
                                                <button
                                                    ...
                                                    className="p-3 bg-white/5 border border-white/10 rounded-xl hover:text-red-500 hover:border-red-500/30 transition-all">
                                                    <Trash2 size={16} />
                                                </button>
                                            </div>
                                        </td>
